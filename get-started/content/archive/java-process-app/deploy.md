...
ENGINE-07015 Detected @ProcessApplication class 'org.cadenzaflow.bpm.getstarted.loanapproval.LoanApprovalApplication'
INFO org.cadenzaflow.commons.logging.BaseLogger.logInfo
...
Now use Cockpit to check if the process is successfully deployed. Go to [http://localhost:8080/cadenzaflow/app/cockpit](http://localhost:8080/cadenzaflow/app/cockpit). Log in with demo / demo. Your process *Loan Approval* is visible on the dashboard.
...
Next, go to Cadenzaflow Tasklist ([http://localhost:8080/cadenzaflow/app/tasklist](http://localhost:8080/cadenzaflow/app/tasklist)). Click on the {{< glyphicon name="list-alt" text=" Start process" >}} button to start a process instance. This opens a dialog where you can select *Loan Approval* from the list. Now you can set variables for the process instance using a generic form.
...
If you now go back to [Cadenzaflow Cockpit](http://localhost:8080/cadenzaflow/app/cockpit), you see the newly created process instance that is waiting in the User Task.

# Configure Process Start Authorizations

To allow the user *john* to see the process definition *Loan Approval*, you have to go to Cadenzaflow Admin ([http://localhost:8080/cadenzaflow/app/admin/default/#/authorization?resource=6](http://localhost:8080/cadenzaflow/app/admin/default/#/authorization?resource=6)). Next, click on the button *Create new authorization* to add a new authorization on the resource *process definition*. Now you can give the user *john* all permissions on process definition *approve-loan*. When you are done, submit the new authorization.
...
Log out of Admin. Go to Tasklist ([http://localhost:8080/cadenzaflow/app/tasklist](http://localhost:8080/cadenzaflow/app/tasklist)) and log back in with the user credentials "john / john". Now you see the *Approve Loan* task in your Tasklist. Select the task and click on the *Diagram* tab. This displays the process diagram highlighting the User Task that is waiting for you to work on it.
